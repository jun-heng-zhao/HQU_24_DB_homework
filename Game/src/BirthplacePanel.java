...
/**
 * BirthplacePanel: 对应原 showBirthplacePanel + free/random choose logic。
 * 修改：角色数据在左上角，适应新布局
 */
public class BirthplacePanel {
    private final AppUI ui;
    private final Database db;
    private final BehaviorService behaviorService;
    private final GameState state;

    public BirthplacePanel(AppUI ui, Database db, BehaviorService behaviorService, GameState state) {
        this.ui = ui;
        this.db = db;
    ...
    public JPanel createPanel() {
        ...
        JLabel title = new JLabel("第一步：选择出生地");
    ...
    private void freeChooseProvince() {
        try {
            ResultSet rs = behaviorService.getProvincesResultSet();
            DefaultListModel<String> listModel = behaviorService.loadProvincesListModel();
            JList<String> provinceList = new JList<>(listModel);
            provinceList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
            provinceList.setVisibleRowCount(12);
            provinceList.setFont(new Font("微软雅黑", Font.PLAIN, 14));

            int choice = JOptionPane.showOptionDialog(
                    ui.getFrame(), new JScrollPane(provinceList), "选择省份",
                    JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null, null, null
            );

            if (choice == JOptionPane.OK_OPTION && provinceList.getSelectedIndex() != -1) {
                rs.absolute(provinceList.getSelectedIndex() + 1);
                ...
                // 添加选择记录到历史
                String provinceName = listModel.getElementAt(provinceList.getSelectedIndex()).split("\\. ")[1].split(" \\(")[0];
                state.addBehaviorHistory("出生地: " + provinceName);

                ui.showBackgroundPanel();
            }
        } catch (SQLException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(ui.getFrame(), "省份选择失败: " + e.getMessage(), "错误", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void randomChooseProvince() {
        try {
            ResultSet rs = behaviorService.getProvincesResultSet();
            int randomIndex = behaviorService.randomIndexFromResultSet(rs);
            if (randomIndex != -1) {
                rs.absolute(randomIndex);
                String provinceName = rs.getString("template_name");
                state.addConnections(rs.getInt("connections"));
                state.addIntelligence(rs.getInt("intelligence"));
                state.addPhysique(rs.getInt("physique"));
                state.addWealth(rs.getInt("wealth"));
